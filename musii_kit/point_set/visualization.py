...
class ScoreVisualization:
    ...
    def mark_occurrences(
        self,
        pattern_occurrences: PatternOccurrences2d,
        pattern_color='red',
        occurrence_colors=['red'],
        discard_unisons=True,
    ):
        """
        ...
        for i in range(len(pattern_occurrences.occurrences)):
            self.mark_pattern(
                pattern_occurrences.occurrences[i],
                occurrence_colors[i % len(occurrence_colors)],
    ...
    def get_selection(self) -> m21.stream.Stream:
    ...
    def _repr_html_(self):
        return display.Notation(self.score)._repr_html_()
